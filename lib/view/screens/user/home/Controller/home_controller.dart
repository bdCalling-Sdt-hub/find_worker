...
class UserHomeController extends GetxController {
  ...
  //
  RxList<UserByServiceModel> allServiceList = <UserByServiceModel>[].obs;
  ...
  getCategory() async {
    try {
      var result =
          await firebaseStorage.collection(AppConstants.category).get();
      categoryList.value = List<CategoryModel>.from(
          result.docs.map((x) => CategoryModel.fromJson(x)));
  ...
  Future<List<UserByServiceModel>?>
      getSortedServicesByAverageRatingCarWash() async {
    final servicesCollection = FirebaseFirestore.instance
        .collection(AppConstants.services)
        .where("category_id", isEqualTo: "VBhuTZnI7ec0Bvy5IYIm");
  ...
  Future<List<UserByServiceModel>?>
      getSortedServicesByAverageRatingHomeClean() async {
    final servicesCollection = FirebaseFirestore.instance
        .collection(AppConstants.services)
        .where("category_id", isEqualTo: "exKagBPgKKUVQ8QyYrO7");
    final usersCollection =
        FirebaseFirestore.instance.collection(AppConstants.users);
  ...
  Future<List<UserByServiceModel>?>
      getSortedServicesByAverageRatingAirCondition() async {
    final servicesCollection = FirebaseFirestore.instance
        .collection(AppConstants.services)
        .where("category_id", isEqualTo: "b0yWTYORILumrLmVpxrt");
    final usersCollection =
        FirebaseFirestore.instance.collection(AppConstants.users);
  ...
  Future<List<UserByServiceModel>?> getAllServiceList() async {
    final servicesCollection = FirebaseFirestore.instance
        .collection(AppConstants.services)
